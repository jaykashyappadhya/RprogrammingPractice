...
Games["JoeJohnson","2007"]
Games["2007","JoeJohnson"]
tgames[]<-t(Games)
tgames["2007","JoeJohnson"]
is.matrix(tgames)
matplot(FieldGoals)
...
Games
Games[,c("2008","2009")]
#If you want to see the matrix, not vector then we have to use the drop=F command because 
#by default, the [] will try to get vector first and then matrix

Games[1,,drop=F] 
trial <- readline(prompt = "Enter an integer:")
datarow<- as.integer(trial)
if(is.na(datarow)){
  mydata <- function(datarow)
  Data <- MinutesPlayed[datarow,,drop=F]
  matplot(t(Data),type = "b",pch = 15:18,col=c(1:4,6))
  legend("bottomleft",inset = 0.01,legend = Players[datarow],col=c(1:4,6),pch = 1:18)
}
datarow <- 
playerdata <- mydata(datarow)
print(playerdata)
...
read.csv(source)
getwd()
setwd("E://Semester3/DATA/")
stats <- read.csv(file = "DemographicData.csv")
head(stats, n = 10)
test= stats$Country.Name=="Malta"
is.vector(test)
...
qplot(data= stats,x= Income.Group, y = Birth.rate, 
      size = I(4), colour = I("blue"))

qplot(data= stats,x= Internet.users , y = Country.Name, 
      size = I(3), colour = Birth.rate)
...
mydata <- data.frame(countries = Countries_2012_Dataset,
...
mydataset <-merge.data.frame(stats,mydata,by.x = "Country.Code",by.y = "Codes")

mydataset$Country.Name=NULL
print(mydataset)

#------------ Now visualization for the data set
